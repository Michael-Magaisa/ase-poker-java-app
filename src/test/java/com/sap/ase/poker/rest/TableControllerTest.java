...
public class TableControllerTest {
  ...
  @MockBean PlayerNamesRepository playerNamesRepository;
  ...
  void getTable_returnsGetTableResponseDtoWithTableStatus() throws Exception {
    ...
    Mockito.when(tableService.getPlayers())
        .thenReturn(
            Arrays.asList(new Player(ALICE_ID, "Alice", 100), new Player("bob", "Bob", 100)));
    Mockito.when(tableService.getState()).thenReturn(GameState.FLOP);

    MockHttpServletResponse response =
        mockMvc
            .perform(get(PATH).principal(mockPrincipal))
            .andExpect(status().isOk())
            .andReturn()
            .getResponse();

    GetTableResponseDto result =
        objectMapper.readValue(response.getContentAsString(), GetTableResponseDto.class);
  ...
  void joinTable_adsValidPlayerToTable() throws Exception {
    ...
    mockMvc
        .perform(post(PATH + "/players").principal(mockPrincipal))
  ...
  void start_startsGameWithTableService() throws Exception {
    ...
    mockMvc
        .perform(post(PATH + "/start").principal(mockPrincipal))
        .andExpect(status().isNoContent())
        .andReturn()
        .getResponse();
