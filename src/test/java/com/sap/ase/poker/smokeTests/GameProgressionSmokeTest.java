...
class GameProgressionSmokeTest extends SmokeTest {
  ...
  void addPlayer_shouldAddNewPlayer() {
    ...
    assertThat(result.getPlayers().stream().map(PlayerDto::getName))
        .containsOnly(BILL_NAME, ALICE_NAME);
  ...
  void startGame_shouldStartGame() {
    ...
    assertThat(result.getPot()).isZero();
  ...
  void playerActionsShouldProgressGameState() {
    assertThat(getTableResponseDtoForPlayer(ALICE_ID).getState())
        .isEqualTo(GameState.OPEN.getValue());

    addTwoPlayers();
    startGame();

    assertThat(getTableResponseDtoForPlayer(ALICE_ID).getState())
        .isEqualTo(GameState.PRE_FLOP.getValue());
    assertThat(getTableResponseDtoForPlayer(ALICE_ID).getCommunityCards()).isEmpty();

    performAction(checkActionAsString());
    performAction(checkActionAsString());

    assertThat(getTableResponseDtoForPlayer(ALICE_ID).getState())
        .isEqualTo(GameState.FLOP.getValue());
    ...
    assertThat(getTableResponseDtoForPlayer(ALICE_ID).getState())
        .isEqualTo(GameState.TURN.getValue());
    ...
    assertThat(getTableResponseDtoForPlayer(ALICE_ID).getState())
        .isEqualTo(GameState.RIVER.getValue());
    ...
    assertThat(getTableResponseDtoForPlayer(ALICE_ID).getState())
        .isEqualTo(GameState.ENDED.getValue());
  }
